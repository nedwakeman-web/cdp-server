...
// ── CORS — allow Netlify and custom domains ──
app.use((req, res, next) => {
  const allowed = [
    'https://cosmicdailyplanner.com',
    'https://cosmicdailyplanner-beta.netlify.app',
    'http://localhost:3000',
    'http://localhost:3001',
  ];
  const origin = req.headers.origin;
  if (!origin || allowed.includes(origin) || origin.endsWith('.netlify.app') || origin.endsWith('.up.railway.app')) {
...
function callAPI(model, maxTok, sys, user) {
  return new Promise((resolve, reject) => {
    ...
    // 55s socket timeout — belt and braces, but streaming keeps it alive
    req.setTimeout(55000, () => {
      req.destroy(new Error('Socket timeout after 55s'));
    });
    req.write(body);
    req.end();
  });
...
Generate the FULL ORACLE READING as valid JSON (no markdown, no fences, no preamble — JSON object only):
{
  ...
  // 6000 tokens — streaming keeps connection alive, no timeout risk
  const raw = await callAPI('claude-sonnet-4-20250514', 6000, sys, user);
